...
const equalSuitedHands = (card, cardHand) =>
  card.length > 3 &&
...
const equalOffsuitedHands = (card, cardHand) =>
  card.length > 3 &&
  cardHand.getHand().substr(0, 1) === card.substr(0, 1) &&
  cardHand.getHand().substr(1, 1) === card.substr(2, 1) &&
...
const equalPairsSpecificCombos = (card, cardHand) =>
  cardHand.getHand().length === 2 &&
  cardHand.getHand().substring(0, 1) == card.substring(0, 1) && //Could be comparing number to string
