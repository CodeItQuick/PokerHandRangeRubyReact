...
const Board = ({ onMouseOverHandler, SelectedRanges, preflopRanges }) => {
	...
	const bind = useGesture({
			...
			onMouseOverHandler(
				{
					...
					onMouseDownEvent:
						(props.memo !== props.args[props.args.length - 1] && props.down) || (props.first && props.down)
				},
		...
		onMove: (props) =>
			onMouseOverHandler(
				{
					cards: props.args[props.args.length - 1],
					onMouseDownEvent: (props.memo !== props.args[0] && props.down) || (props.first && props.down)
				},
	...
	useEffect(
		() => {
			if (!instanceOfBoardHands) updateInstanceOfBoardHands(new BoardOfHands(bind));
