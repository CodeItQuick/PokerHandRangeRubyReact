...
export class CardHandSuitBuilder {
  build(cardOnePar, cardTwoPar, suitPar = "") {
    ...
    if (cardOnePar.length === 2) {
      createdSuitHand = this.buildWithCardOneTwoAndNoSuit(
        cardOnePar,
        cardTwoPar
      );
    }
    ...
    if (suitPar.length === 2 && cardOnePar.length === 1) {
      createdSuitHand = this.buildWithCardOneAndSuitCardTwoAndSuit(
        cardOnePar,
        cardTwoPar,
        suitPar
      );
    ...
    if (suitPar.length === 0 && cardOnePar.length === 1) {
      createdSuitHand = this.buildWithCardOneCardTwoAndSpecificHand(
        cardOnePar,
        cardTwoPar
      );
    }
    ...
    if ((suitPar.length === 1) & (cardOnePar.length === 1)) {
      ...
      createdSuitHand = new StartingHand(cardOrder[0], cardOrder[1], suitPar);
    }
  ...
  assignSuit(suitPar, cardOnePar, cardTwoPar) {
    ...
    if (suitPar.length < 1)
      suit = this._displayCardSuit(cardOnePar, cardTwoPar);
    if (suitPar === 0) suit = "";
    return suit;
  }
  ...
  buildWithCardOneAndSuitCardTwoAndSuit(cardOnePar, cardTwoPar, suitPar) {
    const optionalCardSuitOne =
      suitPar.length > 1 ? suitPar.substring(0, 1) : "";
    const optionalCardSuitTwo =
      suitPar.length > 1 ? suitPar.substring(1, 2) : "";
  ...
  buildWithCardOneTwoAndNoSuit(cardOne, cardTwo) {
    ...
    return new StartingHand(cardOneRank, cardTwoRank, combinedSingleHandSuit);
  }
