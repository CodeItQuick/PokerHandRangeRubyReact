...
interface MyRailsPipelineStackProps {
    vpc: ec2.IVpc,
    dbUrl: string,
    repoName: string,
...
export class MyRailsPipelineStack extends cdk.Stack {
    constructor(scope: cdk.App, id: string, props: MyRailsPipelineStackProps) {
        super(scope, id);

        const pipeline = new codepipeline.Pipeline(this, 'FargatePipeline', {
            pipelineName: 'MyRailsPipeline',
        });

        const repo = new codecommit.Repository(this, 'CodeCommitRepo', {
            repositoryName: '..',
            description: 'created by aws-rails-provisioner with AWS CDK for MyRails'
        });
        ...
        const migration = new codebuild.PipelineProject(this, 'DBMigration', {
            projectName: 'MyRailsDBMigration',
